      ...
      <div className="mx-auto w-full max-w-6xl px-5 py-24 md:px-8 md:py-32">
        ...
        <ul className="grid gap-6 md:grid-cols-3">
          {capabilities.map((item) => (
            <li key={item.title} className="rounded-3xl border border-white/8 bg-card p-7">
              <h3 className="text-lg font-semibold tracking-tight">{item.title}</h3>
              <p className="mt-4 text-sm leading-7 text-muted">{item.description}</p>
            </li>
          ))}
        </ul>
        <dl className="mt-12 grid gap-6 border-t border-white/6 pt-10 text-sm text-muted sm:grid-cols-3">
          <div>
