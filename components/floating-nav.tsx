...
export function FloatingNav() {
  ...
  return (
    <nav
      aria-label="섹션 내비게이션"
      className="fixed bottom-5 left-1/2 z-40 w-fit max-w-[calc(100vw-1.5rem)] -translate-x-1/2 rounded-full border border-border bg-nav px-1.5 py-1.5 shadow-lg shadow-foreground/10 backdrop-blur-xl sm:px-2 sm:py-2"
    >
            ...
            <li key={item.href} className="shrink-0">
              <Link
                ...
                className={`block whitespace-nowrap rounded-full px-2.5 py-2 text-xs leading-none [word-break:keep-all] transition-colors sm:px-4 sm:text-sm ${
                  isActive ? "bg-foreground/10 text-foreground" : "text-muted hover:text-foreground"
                }`}
              >
                {item.label}
              </Link>
